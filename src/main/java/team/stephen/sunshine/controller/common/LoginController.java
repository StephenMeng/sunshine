...
public class LoginController extends BaseController {
    ...
    public Response login(@RequestParam(value = "userName") String userNo,
        ...
        try {
            ...
            LogRecord.info("对用户[" + userNo + "]进行登录验证..验证开始");
            currentUser.login(token);
            LogRecord.info("对用户[" + userNo + "]进行登录验证..验证通过");
        } catch (UnknownAccountException uae) {
            LogRecord.info("对用户[" + userNo + "]进行登录验证..验证未通过,未知账户");
            return Response.error(400, "error", "未知账户");
        } catch (IncorrectCredentialsException ice) {
            LogRecord.info("对用户[" + userNo + "]进行登录验证..验证未通过,错误的凭证");
            return Response.error(400, "message", "密码不正确");
        } catch (LockedAccountException lae) {
            LogRecord.info("对用户[" + userNo + "]进行登录验证..验证未通过,账户已锁定");
            Response.error(400, "message", "账户已锁定");
        } catch (ExcessiveAttemptsException eae) {
            LogRecord.info("对用户[" + userNo + "]进行登录验证..验证未通过,错误次数过多");
            Response.error(400, "message", "用户名或密码错误次数过多");
        } catch (AuthenticationException ae) {
            //通过处理Shiro的运行时AuthenticationException就可以控制用户登录失败或密码错误时的情景
            LogRecord.info("对用户[" + userNo + "]进行登录验证..验证未通过,堆栈轨迹如下");
            ae.printStackTrace();
